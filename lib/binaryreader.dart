...
class BinaryReader {
  ...
  final Map<int, AssetClass> assetClassTable;

  bool get done => _index >= _rawData.length;

  int readUint8() {
    if (_index > _rawData.length - 1)
      throw StateError('called readUint8 without enough bytes to read a uint8');
    _index += 1;
  ...
  int readInt32() {
    if (_index > _rawData.length - 4)
      throw StateError(
        'called readInt32 without enough bytes to read an int32',
      );
    _index += 4;
  ...
  Uint64 readUint64() {
    return endian == Endian.big
        ? Uint64.bigEndian(readUint32(), readUint32())
        : Uint64.littleEndian(readUint32(), readUint32());
  }
  ...
  String readString() {
    ...
    if (length == 0) {
      throw StateError(
        'readString: got empty string with identifier that isn\'t 0',
      );
    }
  ...
  AssetClass? readAssetClass(bool noZero) {
    int id = readInt32();
    if (id != 0) {
      AssetClass? result = assetClassTable[id];
      if (result != null) {
        return result;
      }
    } else if (noZero) {
